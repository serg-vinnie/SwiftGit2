...
public extension Duo where T1 == Branch, T2 == RemoteRepo {
	...
	func push(credentials1: Credentials =  .sshAgent) -> Result<(), NSError> {
		...
		//let credentials = Credentials
			//.plaintext(username: "UKS", password: "Sr@mom!Hl3dr:gi")
			//.sshUks(username: "skulptorrr@gmail.com", passphrase: "")
			//.sshMemory(username: "skulptorrr@gmail.com", publicKey: sshMan.urlPublicKey.path,
			//									   privateKey: sshMan.urlPrivateKey.path, passphrase: "")
		
		let credentials = try! CredGenerate().get()
		var opts = pushOptions2(credentials: credentials)
		
		
//		let credentialsStandard = Credentials.fromPointer(credentials.pointer!)
//		var opts = pushOptions(credentials: credentialsStandard)
...
fileprivate func pushOptions2(credentials: CredentialSsh) -> git_push_options {
	let pointer = UnsafeMutablePointer<git_push_options>.allocate(capacity: 1)
	git_push_init_options(pointer, UInt32(GIT_PUSH_OPTIONS_VERSION))
	...
	options.callbacks.payload = credentials.toPointer()
	options.callbacks.credentials = credentials.toCallBack()  //git_credential()  //credentialsCallback
