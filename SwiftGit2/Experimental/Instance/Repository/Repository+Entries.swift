...
public extension Repository {
    ...
    func commitDetails(parents: [OID], deltas:[[Diff.Delta]], desc: String) -> R<CommitDetails> {
        ...
        if let firstDelta = filetredDeltas.first {
            return filteredAll | { CommitDetails(parents: filteredParents, deltas: firstDelta, all: $0, desc: desc) }
        }
        
        return .success(CommitDetails(parents: [], deltas: [], all: [:], desc: desc))
