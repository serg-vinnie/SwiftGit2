...
public extension Repository {
    ...
    func commit(branch: String) -> R<Commit> {
        reference(name: branch) | { $0.target_result } | { self.instanciate($0)}
    }
    ...
    func mergeAndCommit(anal: MergeAnalysis, our: Branch, their: Branch, signature: Signature, options: MergeOptions, stashing: Bool = false) -> Result<MergeResult, Error> {
        ...
        if anal.contains(.fastForward) || anal.contains(.unborn) {
            ...
            let theirOID = their.target_result
...
public struct MergeAnalysis: OptionSet {
    ...
    nonisolated(unsafe) public static let none = MergeAnalysis(rawValue: GIT_MERGE_ANALYSIS_NONE.rawValue) // No merge is possible.  (Unused.)
    nonisolated(unsafe) public static let normal = MergeAnalysis(rawValue: GIT_MERGE_ANALYSIS_NORMAL.rawValue) // A "normal" merge; both HEAD and the given merge input have diverged from their common ancestor. The divergent commits must be merged.
    nonisolated(unsafe) public static let upToDate = MergeAnalysis(rawValue: GIT_MERGE_ANALYSIS_UP_TO_DATE.rawValue) // All given merge inputs are reachable from HEAD, meaning the repository is up-to-date and no merge needs to be performed.
    nonisolated(unsafe) public static let fastForward = MergeAnalysis(rawValue: GIT_MERGE_ANALYSIS_FASTFORWARD.rawValue) // The given merge input is a fast-forward from HEAD and no merge needs to be performed. Instead, the client can check out the given merge input.
    nonisolated(unsafe) public static let unborn = MergeAnalysis(rawValue: GIT_MERGE_ANALYSIS_UNBORN.rawValue) // The HEAD of the current repository is "unborn" and does not point to a valid commit. No merge can be performed, but the caller may wish to simply set HEAD to the target commit(s).
}
...
public struct MergePreference: OptionSet {
    ...
    nonisolated(unsafe) public static let none = MergeAnalysis(rawValue: GIT_MERGE_PREFERENCE_NONE.rawValue) // No configuration was found that suggests a preferred behavior for merge.
    nonisolated(unsafe) public static let noFastForward = MergeAnalysis(rawValue: GIT_MERGE_PREFERENCE_NO_FASTFORWARD.rawValue) // There is a merge.ff=false configuration setting, suggesting that the user does not want to allow a fast-forward merge.
    nonisolated(unsafe) public static let fastForwardOnly = MergeAnalysis(rawValue: GIT_MERGE_PREFERENCE_FASTFORWARD_ONLY.rawValue) // There is a merge.ff=only configuration setting, suggesting that the user only wants fast-forward merges.
}
