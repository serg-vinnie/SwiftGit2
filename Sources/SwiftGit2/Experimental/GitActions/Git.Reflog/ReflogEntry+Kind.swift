...
nonisolated(unsafe) var resetParser = Parse {
    StartsWith("reset: moving to")
    Rest<String.SubSequence>()
}

nonisolated(unsafe) var commitParser = Parse {
    StartsWith("commit: ")
    Rest<String.SubSequence>()
}

nonisolated(unsafe) var commitMergeParser = Parse {
    StartsWith("commit (merge): ")
    Rest<String.SubSequence>()
}

nonisolated(unsafe) var commitInitParser = Parse {
    StartsWith("commit (initial): ")
    Rest<String.SubSequence>()
}

nonisolated(unsafe) var pullParser = Parse {
    StartsWith("pull: ")
    Rest<String.SubSequence>()
}

nonisolated(unsafe) var cloneParser = Parse {
    StartsWith("clone: from ")
    Rest<String.SubSequence>()
}

nonisolated(unsafe) var checkoutParser = Parse {
